 ...
 *
 * SVN $Revision: 5577 $
 * SVN $Date: 2013-04-04 10:23:27 +0100 (Thu, 04 Apr 2013) $
 */
#include "mss_spi.h"
...
)
{
    ...
    tx_fifo_full = this_spi->hw_reg->STATUS & TX_FIFO_FULL_MASK;
    while((0u == tx_fifo_full) && (this_spi->slave_tx_idx < tx_buff_size))
    {
...
)
{
    ...
    tx_size = this_spi->slave_tx_size;
    tx_fifo_full = this_spi->hw_reg->STATUS & TX_FIFO_FULL_MASK;
    
    while((0u == tx_fifo_full) && (1u == more_data))
    {
        if(this_spi->slave_tx_idx < tx_size)
        {
            this_spi->hw_reg->TX_DATA = this_spi->slave_tx_buffer[this_spi->slave_tx_idx];
            ++this_spi->slave_tx_idx;
        }
...
)
{
    ...
    else 
    {
        ...
        /* Block handling mode. */
        rx_fifo_empty = this_spi->hw_reg->STATUS & RX_FIFO_EMPTY_MASK;
        rx_size = this_spi->slave_rx_size;
        
        while((0u == rx_fifo_empty) && (this_spi->slave_rx_idx <rx_size))
        {
            rx_frame = this_spi->hw_reg->RX_DATA;
...
)
{
    ...
    if(this_spi->hw_reg->MIS & CMD_IRQ_MASK)
    {
        ...
        /* Disable command interrupt until slave select becomes de-asserted to avoid retrigering. */
        this_spi->hw_reg->CONTROL2 &= ~ENABLE_CMD_IRQ_MASK;
